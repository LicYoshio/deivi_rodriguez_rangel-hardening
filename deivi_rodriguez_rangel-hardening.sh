...
#Iniciamos un if para condicionar si posee un OS Ubuntu o CentOS
if grep debian /etc/os-release 1>/dev/null ; then
	echo "OS = Ubuntu."
	if apt list --installed 2>/dev/null | grep clamav 2>/dev/null 1>&2 ; then
		echo "\e[1;31;1m **Desinstalando clamav... \e[0m"
		yes '' | sudo apt autoremove clamav clamav-daemon 2>/dev/null 1>&2
		clear && echo "\e[1;32;1m **Clamav desinstalado. \e[0m"
	fi
	echo "\e[1;31;1m **Instalando clamav... \e[0m"
	yes '' | sudo apt install clamav clamav-daemon 2>/dev/null 1>&2
	clear && echo "\e[1;32;1m **Clamav instalado. \e[0m"

	echo "\e[1;31;1m **Actualizando paquetes... \e[0m"
	yes '' | sudo apt update 2>/dev/null 1>&2 && yes '' | sudo apt upgrade 2>/dev/null 1>&2
	echo "\e[1;32;1m **Paquetes actualizados. \e[0m"

	echo "\e[1;32;1m **Finalizado! \e[0m"

else
	echo "OS = CentOS."
	if ! yum list installed | grep epel-release 1>/dev/null ; then
		echo -e "\e[1;31;1m **Instalando epel-release...\n \e[0m"
		sudo yum -y install epel-release && sudo yum clean all 1>/dev/null
		clear && echo "\e[1;32;1m **epel-release instalado. \e[0m"
	fi

	if yum list installed | grep clamav 1>/dev/null ; then
		echo -e "\e[1;31;1m **Desinstalando clamav... \e[0m"
		...
		clear && echo -e "\e[1;32;1m **Clamav desinstalado. \e[0m"
	fi
	echo -e "\e[1;31;1m **Instalando clamav... \e[0m"
	...
	clear && echo -e "\e[1;32;1m **Clamav instalado. \e[0m"

	echo -e "\e[1;31;1m **Actualizando paquetes... \e[0m"
	sudo yum -y update 1>/dev/null
	echo -e "\e[1;32;1m **Paquetes actualizados. \e[0m"
	echo -e "\e[1;32;1m **Finalizado! \e[0m"
fi
